...
/*
// Fujitsu codes dans la librairy.
          ...
          if (currentLine.length() == 0) {
            ...
            sendIRCommand(command);
              ...
              if (temperaturePosition != -1) {
                ...
                String temperatureValue = header.substring(temperaturePosition + 12); // 12 est la longueur de "temperature="             
              ...
              if (vdirPosition != -1) {
                // Extraire la valeur de la position.
                String vdirValue = header.substring(vdirPosition + 5); // 5 est la longueur de "vdir=".
                // Supprimer la partie " HTTP/1.1" de la chaîne
...
uint8_t getModeValue(String modeString) {
  modeString.trim(); // Enlève les espace blancs si il en a dans la chaine de caratère modeString.
  if (modeString == "MODE_AUTO") {
    ...
    return MODE_HEAT; // Utiliser MODE_HEAT comme valeur par défaut
  }
}
...
uint8_t getFanValue(String modeString) {
  ...
  if (modeString == "FAN_AUTO") {  
    ...
    // Gérer le cas où le mode n'est pas reconnu, peut-être retourner une valeur par défaut.
    return FAN_4; // Utiliser MODE_HEAT comme valeur par défaut
  }
}
...
uint8_t getVdirValue(String modeString) {
  ...
  if (modeString == "VDIR_MANUAL") {  
    ...
    // Gérer le cas où le mode n'est pas reconnu, peut-être retourner une valeur par défaut.
    return VDIR_MANUAL; // Utiliser VDIR_MANUAL comme valeur par défaut.
  }
}
